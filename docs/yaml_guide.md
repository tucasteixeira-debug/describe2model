...
How to describe the **decision-making and control logic of a real system** in `describe2model`'s YAML format, and how the engine turns that description into something that actually runs.

This is not a generic logic-expression language, and it isn't "logic" in the abstract computer-science sense. It's the same category of thing that actually runs inside a PLC or a microcontroller: the rules that decide when a motor turns on, when a valve opens, when a latch holds and when it lets go — real control decisions, re-evaluated on a fixed cycle, driving a real system's behavior. `and`/`or`/`gt`/`if` and the function blocks below are just the vocabulary that control logic is built out of, the same way ladder logic or structured text would be; they're not the point in themselves.

This guide is also deliberately **not elevator-specific**. The elevator in `example/` is one system controlled this way; the format itself doesn't know or care what it's controlling. Anything whose control logic is expressible as *signals, timers, counters, latches, simple comparisons, and simple math, re-evaluated on a fixed scan* fits the same five sections and the same vocabulary — a tank-fill sequence, a traffic light, a conveyor interlock, a batching process. The physics plants in `example/Elevator_System_2` and `3` are a separate layer bolted on *underneath* the control logic (they simulate the physical world the decisions act on) — the YAML format described here is purely the decision-making layer, with no opinion about physics at all.

## The big picture

A plain-English description of what a system should *do* gets translated into one YAML file: a catalog of **tags** (every real signal the system has — inputs, decisions, commands) and a list of **operations** (the control logic that computes them). You write the operations in whatever order reads best to a human — the engine works out the real execution order itself before running anything, then re-evaluates every operation once per scan, forever, the same way a PLC or microcontroller's scan cycle does: read inputs, decide, act, repeat.

![Scan cycle flow](assets/scan_cycle_flow.svg)

The only thing that persists between scans is the shared `tags` dictionary — the system's current state and its most recent decisions. Nothing else is remembered, which is exactly why the "same-scan cycle" rule below matters as much as it does: it's the same constraint a real scan-based controller lives under, not an arbitrary restriction of this format.
...
## The expression language (JsonLogic-style)
...
Comparison and arithmetic operators accept either a nested node *or* a bare literal in each of their two slots — `{gt: [{var: "X"}, 5]}` is fine. It's only the field itself (`PT`, `IN`, the top of an `expression`) that can never be a bare literal.
...
A **self-oscillating pulse**, real snippet from `example/Elevator_System_1/elevator_1.yaml`:
...
**RS** — Set/Reset latch, Reset-dominant. Fields: `Set` (list, OR'd), `Reset` (list, OR'd). Real snippet:
...
**CTUD** — up/down counter. Fields: `CU`, `CD` (count up/down on rising edge), `R` (reset), `LD` (load), `load_value` (**a bare literal, the one field that is *not* a JsonLogic node**), `PV` (preset/max). Clamped to `[0, PV]`; precedence when several fire the same scan is R beats LD beats CU/CD. Real snippet:
...
**PID** — Fields: `MANUAL`, `Y_MANUAL`, `RESET`, `KP`, `TN`, `TV`, `Y_MIN`, `Y_MAX`, `SET_POINT`, `ACTUAL`. `Y = KP * (error + I/TN + TV*de/dt)` — `KP` scales the whole sum, not just the P term. Anti-windup is back-calculation: the integral term is rewritten (not just frozen) whenever the output saturates, so recovery after a long saturation isn't slow. Not exercised anywhere in the elevator example (a discrete-floor dispatch problem has no continuous setpoint to track), but it's part of the vocabulary for anything that does — a temperature loop, a flow-rate controller.
...
`graph_builder` walks every operation's fields, finds every `{var: "X"}` reference, and adds a dependency edge if `X` is produced by another operation. `topological_sorter` produces a valid order from that graph. You never hand-order the YAML file.

## The one rule that will bite you: same-scan cycles

If operation A reads a tag produced by B, and B (even transitively) reads a tag produced by A, that's a dependency **cycle** — sorting raises `CycleError` rather than silently picking an order. This happens more than it sounds like it should, because "X determines Y, and Y's outcome should affect X" is a very natural thing to want to describe in plain language, and it's exactly what breaks a single-pass evaluation.
...
This computes `max(0, CUSUM_High_prev + Abs_Velocity_Residual - Slack_K)` every scan — the `max(0, x)` clamp built from `if`/`gt` since there's no dedicated `max` primitive. `graph_builder` gives it zero dependency edges, so it creates no cycle risk despite reading its own output every scan.
